...
<table>
	<tr>
		<td colspan="13"><h1>Data Skema Pinjaman</h1></td>
	</tr>
	...
	<?php
		...
		if ($akses == "admin") {
			$wheredivisi = "WHERE b.id_divisi IN ('1','2')";
		}elseif($akses == "sekertaris"){
			// $wheredivisi = "WHERE b.id_divisi = '3' OR b.id_divisi = '4'";
			$wheredivisi = "WHERE b.id_divisi IN ('3','4')";
		}else {
			$wheredivisi = "";
		}
		...
		while($data=mysqli_fetch_array($query))
		{
			...
			$sqlb	  ="SELECT * FROM akun WHERE nm_akun LIKE '%$cari%' AND id=$dataa[11] AND stts_akun NOT LIKE '3'";
	...
	<tr class="hover" bgcolor="<?php echo $warna ?>">
		...
		<?php if ($akses=='superuser' OR $akses=='admin' OR $akses=='sekertaris') { ?>
		<td>
			<?php if ($data[9]==1) { ?>
